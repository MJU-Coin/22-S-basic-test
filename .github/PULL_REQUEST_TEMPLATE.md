## 👩‍💻 요구 사항과 구현 내용
-Side Bar
현재 시각 실시간 변화 구현: 자바 스크립트로 clock함수 구현
clock()에서는 Date()클래스를 이용해 년/월/일 시간/분을 실시간으로 구해
가져온 p태그.innerText를 통해 출력  
window.onload()에서 clock()을 호출해 창이 로드 될 때 마다 시간 동기화
